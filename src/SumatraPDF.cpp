...
static TempStr ToUtf8TempLen(const WCHAR* text, int len) {
    if (!text || len <= 0) {
        return nullptr;
    }
    ...
    char* utf8 = (char*)malloc((size_t)utf8Len + 1);
    if (!utf8) {
        return nullptr;
    }

    int written = WideCharToMultiByte(CP_UTF8, 0, text, len, utf8, utf8Len, nullptr, nullptr);
    if (written <= 0) {
        free(utf8);
        return nullptr;
    }
...
static TempStr GetReadAloudCurrentPageTextTemp(WindowTab* tab) {
    ...
    TempStr text = ToUtf8TempLen(pageText.text, pageText.len);
    FreePageText(&pageText);
